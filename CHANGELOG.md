...
All notable changes to DomainTome (formerly Lore) are documented in this file.
...
### Fixed
- Residual `lore` mentions cleaned up in `.gitignore` comment, `examples/booking-api/demo.tape` narration, and `examples/booking-api/scripts/seed_graph.sql` header comment. No behavior change.

## [0.3.0] — 2026-05-02

### Changed (BREAKING — full rename)

The project has been renamed from **Lore** to **DomainTome**. Every
user-facing surface changes; the schema and graph semantics do not.

| Surface | Before | After |
| --- | --- | --- |
| Brand / package | `lore` / `projectlore` | `domaintome` |
| CLI binary | `lore` | `dt` |
| Slash commands | `/lore:*` | `/dt:*` |
| MCP tools | `lore_*` (17) | `dt_*` (17) |
| Plugin name (marketplace) | `lore` | `domaintome` |
| Data dir | `.lore/` | `.dt/` |
| DB filename | `.lore/lore.db` | `.dt/graph.db` |
| MCP server id | `lore` | `dt` |
| Hooks command | `lore hook-*` | `dt hook-*` |
| Repository | `github.com/YACB2/lore` | `github.com/YACB2/domaintome` |

### Migrating from 0.2.x

The schema is unchanged, but every name on the surface is different.
Recommended path:

1. **Uninstall the old plugin** in Claude Code: `/plugin` → select `lore`
   → Uninstall.
2. **Install the new plugin**:
   `/plugin install domaintome@domaintome` then `/reload-plugins`.
3. **Regenerate** your graph data by running `/dt:init` and/or
   `/dt:bootstrap`. The pre-MVP `.lore/` directories are not preserved
   automatically — this is a hard cut.
4. **Update any project `CLAUDE.md`** references that mention `lore_*`
   tools, `/lore:*` commands, or `.lore/` paths.
5. **Update Claude Code hooks** in `settings.json` if you invoked the
   `lore` binary directly: replace `lore` with `dt`.

If you have valuable graph data you want to preserve, manually:

```bash
mv .lore .dt
mv .dt/lore.db .dt/graph.db
```

This *should* keep working since schema is unchanged, but is not
officially supported and may surface edge cases in audit logs.

### Why the rename

Discoverability. The "lore" namespace in the Claude Code / MCP / dev-tools
ecosystem is broadly shared (see GitHub: `instacart/lore`, multiple
`lore-mcp` repos, etc.). DomainTome is distinctive with zero collisions
and a clearer connection to the product's actual purpose: capturing the
*domain* knowledge of a software project as a queryable *tome*.

## [0.2.3] — 2026-04-30

### Fixed
- CI lint failures: added missing `from typing import Any` import in
  `src/domaintome/graph/schema.py` and reordered import blocks in
  `src/domaintome/cli/main.py` and `tests/test_v01_features.py` to satisfy
  `ruff` I001.
...
- **PostToolUse hook diff-aware on Edit/Write/MultiEdit/NotebookEdit.**
  `dt hook-post-tool-use` now matches the Edit-family tools in
  ...
  the agent to call `dt_update_node`/`dt_add_node`/`dt_add_edge`
...
- **Inferential business-signal classifier** for unmapped paths. When
  ...
  `.dt/state.json`, no per-turn tokens.
- Plugin manifest: `PostToolUse` matcher widened to
  `Bash|Edit|Write|MultiEdit|NotebookEdit`.
- MCP write-tool docstrings (`dt_add_node`, `dt_add_nodes`,
  `dt_update_node`, `dt_add_edge`, `dt_add_edges`) rewritten to
...
### Added
- **`dt_schema` MCP tool** — read-only descriptor of node types,
...
- **`dt_add_node(s)` minimal default response** — returns
...
### Notes for upgrades
- Existing `.dt/graph.db` files are upgraded in place on next open.
...
### Added
- **`/dt:sync` slash command** — closes the loop reconcile cannot:
...
- **`src/domaintome/sync.py` module** — extracts `find_git_repos`,
...
- v0.0.19 made the model *aware* a commit had happened. v0.0.20 gives
  ...
  unreflected commits; running `/dt:sync HEAD~15` should walk the
  whole gap in one session.

## [0.0.19] — 2026-04-25

### Changed
- **`dt-usage` skill description rewritten with explicit triggers** —
...
- **SessionStart hook always injects an operating directive when the
  ...
  reinforcement turn after turn and the `dt-usage` skill drifted out
...
- **PostToolUse hook tracks `git commit` as a Lore checkpoint** — new
  `dt hook-post-tool-use` CLI command, registered under PostToolUse
...
### Added
- **`/dt:stats` slash command** — wrapper over `lore stats` CLI with
  ...
  `/dt:stats` and got "command not found". Oversight corrected.
...
- **`part_of` now allows `module → module`** — the missing triple that
  ...
  testing: the new hierarchical `/dt:init` correctly proposed 2 repos
...
### Changed
- **`/dt:init` rewritten as the single smart setup command.** Merges
  what used to be `/dt:init` + `/dt:bootstrap` into one flow, asks
  ...
  - No matter the path, mandatory verification step applies
    (`dt_audit` count comparison, report FAILURE on mismatch).
- **`/dt:bootstrap` repurposed as the re-scan command.** Explicitly
  requires `/dt:init` to have run first (checks for existing
  modules). Use it to extend the graph with additional
  capabilities/flows after initial setup.

### Added
- **`dt hook-session-start` CLI command**, invoked by the
  ...
  - No DB and the cwd looks like a codebase (has `pyproject.toml`,
    ...
    running `/dt:init`.
  - DB exists but empty → suggests `/dt:init` to seed modules.
...
- User feedback: "why do I have to create `.dt/` manually?" Already
...
- **MCP server now always auto-creates the database.** Dropped the
  "refuse if `.dt/` doesn't exist" half-measure from v0.0.14. Forcing
  users to `mkdir .dt/` before first use was friction without real
  ...
  Net effect: open Claude Code anywhere → `/dt:init` → works. No
  `mkdir`, no `lore init` shell ceremony, no "Reconnect" dance.

## [0.0.14] — 2026-04-23

### Changed
- **MCP server now relaxes `refuse-to-start` when `.dt/` exists.** The
  ...
  - DB missing but parent `.dt/` directory exists → auto-create the
    DB inside it. Creating the `.dt/` folder is an explicit opt-in
    to Lore for this project; auto-creating the DB there is safe.
  - Neither DB nor `.dt/` parent exist → refuse with a clear error
    message suggesting `mkdir .dt/` or `lore init`.
...
### Changed
- **`/dt:bootstrap` caps are now tiered by project size.** The old
...
### Added
- **`lore verify <id>` CLI + `/dt:verify <id>` slash command.**
  ...
  `dt_update_node(metadata_patch=…)` call by hand.
...
- **`/dt:show` renders nodes as readable Markdown briefings**:
...
- This release moves Lore closer to the stated vision: each node
  ...
  code. `/dt:show` is now useful as a standalone briefing tool.
...
### Changed
- **`/dt:bootstrap` now defaults to Sonnet**, not Haiku, for the
  ...
  opt-in via `.dt/config.json → models.exploration: "haiku"`.
...
- Model routing audit done for all slash commands. Read-only commands
  (`/dt:audit`, `/dt:reconcile`, `/dt:probe`, `/dt:recent`)
  continue on Haiku — they cannot produce bad data. Write-path
  commands (`/dt:init`, `/dt:bootstrap`) already ran on the
...
- **`SessionStart` plugin hook.** On every new Claude Code session with
  ...
  local `.dt/graph.db`. Silent when there is no DB and when the graph
...
### Added
- `lore reconcile` CLI command + `/dt:reconcile` slash command
  ...
  be wired into hooks.
- `src/domaintome/lifecycle.py` module holding the reconcile engine (8 new
  tests).

### Fixed
- `/dt:init` and `/dt:bootstrap` now require a mandatory
  verification step before claiming success: Claude must call
  `dt_list` / `dt_audit` after persisting, compare the returned
  ...
  testing where `/dt:init` claimed to have seeded 7 modules without
  ever calling `dt_add_nodes`.
...
### Fixed
- Slash commands now resolve as `/dt:init`, `/dt:audit`, `/dt:probe`, etc. instead of the doubled-namespace `/dt:lore:init`. The command files lived in `plugins/lore/commands/lore/` which Claude Code interpreted as an extra namespace level. Flattened to `plugins/lore/commands/`.

## [0.0.5] — 2026-04-22

### Added
- `/dt:probe <path>` slash command (model: haiku) — audit a Lore graph that
...
- `lore audit --json` for structured output (consumed by `/dt:probe`).
...
- **MCP server refuses to start against a missing DB.** Prevents the common
  ...
  silently land in a fresh `.dt/graph.db` at the wrong location. The server
  now logs the resolved DB path on startup.

### Changed
- Plugin `.mcp.json` simplified: the `--db .dt/graph.db` arg was redundant
...
- Documentation and slash commands clarified to state that Lore's "project"
  ...
  `SKILL.md`, `README.md`, `/dt:init` and `/dt:bootstrap` now explicitly
  describe both layouts; `/dt:bootstrap` auto-detects `single_repo` vs
