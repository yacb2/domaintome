"""Bootstrap a Lore graph that models Lore itself — used for dogfooding.

Run:

    uv run python examples/dogfood_lore.py

Creates `.lore/lore.db` at the repo root and populates it with nodes describing
the architecture of the `lore` package (modules, capabilities, flows, entities,
rules, decisions).
"""
...
def seed(db_path: Path = DB_PATH) -> None:
    ...
    add_node(
        ...
        type="flow",
        title="Initialize an empty Lore database",
        body="Create .lore/lore.db, apply the schema, enable foreign keys.",
    )
    add_node(
        conn,
    ...
    add_node(
        ...
        type="event",
        title="audit.failed",
        body="Fired when `lore audit` finds structural issues.",
    )
    ...
    add_node(
        ...
        type="form",
        title="lore_add_node MCP tool input",
    ...
    add_node(
        ...
        type="form",
        title="lore_add_edge MCP tool input",
    ...
    add_node(
        ...
        body="Enforced by lore.graph.schema.ALLOWED_RELATIONS at write time.",
    )
    add_node(
        conn,
