---
description: Detect drift between code and Lore — dead source_refs, stale nodes, never-verified nodes. Read-only.
allowed-tools: [Bash]
model: haiku
argument-hint: [git-rev]
---

Run `lore reconcile` to surface drift between the graph and the current
state of the code. Read-only — never fixes anything automatically. The
user decides what to apply afterwards via targeted `lore_update_node`
calls.
...
4. **Suggest concrete next actions** for each finding:
   ...
   - `stale`: "Re-verify by inspecting the referenced code, then
     `lore_update_node(id, metadata_patch={'last_verified_at': '<today>'})`."
