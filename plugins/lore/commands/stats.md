---
description: Show Lore MCP usage analytics — calls, tokens exchanged, breakdown by tool and op. Optional --since <ISO timestamp>.
argument-hint: [since-iso]
allowed-tools: [Bash]
model: haiku
---

Report Lore's own usage analytics: how many MCP calls have been made
against this graph, the byte volume exchanged (with a rough token
estimate), and a per-tool / per-op breakdown.

## Steps

1. Run `lore stats` against the current project's DB. If `$ARGUMENTS`
   contains an ISO timestamp, pass it as `--since`:

   ```bash
   lore stats [--since $ARGUMENTS]
   ```
...
2. Summarize the output in a tight block:
   ...
   ### By tool
   - lore_add_nodes: N calls · X KB (~Y tokens)
   - lore_audit: N calls · X KB (~Y tokens)
   - ...
...
- After a `/lore:bootstrap` / `/lore:init` to see what it cost.
